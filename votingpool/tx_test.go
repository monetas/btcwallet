...
func (w *Withdrawal) fulfilOutputs() error {
	...
	for _, tx := range w.transactions {
		...
		// TODO: At this point we should also store the transactions in the txstore.
		// Need to think about where to store the signatures, though.
	}
	return nil
}
	...
	for _, tx := range w.transactions {
		...
		for idx := range tx.TxIn {
			...
			if class != btcscript.ScriptHashTy {
				// XXX: Is it ok to assume pkScript is always a P2SH here?
...
func SignMultiSigUTXO(mgr *waddrmgr.Manager, tx *btcwire.MsgTx, idx int, sigs []rawSig) error {
	...
	class, addresses, _, err := btcscript.ExtractPkScriptAddrs(credit.TxOut().PkScript, netParams)
	if err != nil {
		panic(err) // XXX: Again, no idea what's the correct thing to do here.
	}
	if class != btcscript.ScriptHashTy {
...
func validateMsgTx(msgtx *btcwire.MsgTx) error {
	...
	for i, txin := range msgtx.TxIn {
		// FIXME: Must talk to conformal to discuss alternatives to store.UnspentOutput()
